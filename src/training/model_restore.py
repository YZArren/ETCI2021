...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    ...
    search_in = join(os.path.dirname(os.path.dirname(__file__)), "training","network_training")

    tr = recursive_find_python_class([search_in], name, current_module="src.training.network_training")
    ...
    """if len(init) == 7:
        print("warning: this model seems to have been saved with a previous version of  Attempting to load it "
