...
from src.postprocessing.connected_components import load_remove_save, load_postprocessing
from src.training.model_restore import load_model_and_checkpoint_files
...
def check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities):
    ...
    maybe_case_ids = np.unique([i[:-6] for i in files])
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    list_of_lists = [[join(input_folder, i) for i in all_files if i[:len(j)].startswith(j) and
                      len(i) == (len(j) + 6)] for j in case_ids]
